          ...
          children: [
            TextSpan(
              children: [
                // TextSpan(
                //   text: listIndex == -1
                //       ? ("${unorderedListItemShapes[(depth - 1) % unorderedListItemShapes.length]} ")
                //       : "${getOrderedIndex(listIndex + 1, depth)}. ",
                // ),
                children.first.rendererWidget,
              ],
            ),
...
class TextJsonContent extends JsonContent {
  ...
  TextSpan getRichTextFromTicks() {
    int index = 0;
    List<InlineSpan>? children = [];

    for (String splitText in content.split('`')) {
      children.add(
        (index++).isOdd
            ? WidgetSpan(
                child: Card.outlined(
            ...
            : TextSpan(text: splitText, style: textStyle),
      );
    }
    return TextSpan(text: "", style: textStyle, children: [...children]);
  }

  @override
  TextSpan get rendererWidget => getRichTextFromTicks();
}
